...
Опции install:
  ...
  --apt-wait <сек>      сколько ждать чужой apt, если блокировку держит не
                        автообновление (по умолчанию 120)
...
M[apt_unit_stopped]="остановлен %s"
M[apt_foreign_holder]="блокировку держит %s (pid %s) — это не автообновление, жду, а не снимаю"
M[apt_repair]="привожу dpkg в порядок после прерванного обновления"
M[apt_timers_restored]="таймеры автообновления возвращены"

M[apt_disarmed]="автообновление остановлено, жду освобождения блокировки"
