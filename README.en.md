...
- **A fresh VPS is busy with automatic updates.** For the first minutes after boot
  `unattended-upgrades` holds the dpkg lock and `apt-get` fails. The script does not wait:
  it stops automatic updates cleanly and repairs dpkg with `dpkg --configure -a`.
