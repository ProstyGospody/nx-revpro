...
только вы поменяете порт или путь в UI. Оттуда же берутся логин и пароль для API — из
таблицы `users`. К `install-result.env` скрипт обращается только как к запасному источнику:
этот файл пишется один раз при установке и не знает о смене пароля в UI.
